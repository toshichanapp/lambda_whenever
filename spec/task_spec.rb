...
RSpec.describe LambdaWhenever::Task do
  let(:task) { LambdaWhenever::Task.new("production", false, "bundle exec", "cron(0 17 * * ? *)") }
  ...
  describe "#rake" do
    ...
    context "when unset bundle command" do
      let(:task) { LambdaWhenever::Task.new("production", false, "", "cron(0 17 * * ? *)") }
    ...
    context "when set verbose flag" do
      let(:task) { LambdaWhenever::Task.new("production", true, "bundle exec", "cron(0 17 * * ? *)") }
  ...
  describe "#runner" do
    it "generates rails runner commands" do
      ...
      expect(task.commands).to eq([%w[bundle exec rails runner -e production Hoge.run]])
    end
    ...
    context "when unset bundle command" do
      ...
      it "generates rake commands" do
        task.runner("Hoge.run")
        expect(task.commands).to eq([%w[rails runner -e production Hoge.run]])
      end
    end
  end
  ...
  describe "unsupported method" do
    it "does not change commands" do
      ...
      end.not_to(change { task.commands })
    end
  end
end
